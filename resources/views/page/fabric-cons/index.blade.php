            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table align-middle table-nowrap table-hover">
                        ...
                        <tbody>
                            @foreach ($fabricConss as $fabricCons)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $fabricCons->name }}</td>
                                <td>{{ $fabricCons->description }}</td>
                                <td>
                                    <a href="javascript:void(0);" class="btn btn-primary btn-sm btn-edit-fabricCons" data-id="{{ $fabricCons->id }}" data-url="{{ route('fabric-cons.show', $fabricCons->id) }}">Edit</a>
                                    <a href="javascript:void(0);" class="btn btn-danger btn-sm btn-delete-fabricCons" data-id="{{ $fabricCons->id }}" data-url="{{ route('fabric-cons.destroy', $fabricCons->id) }}">Delete</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
...
$(document).ready(function(){
    ...
    $(".btn-delete-fabricCons").on('click', function(e) {
        if(!confirm('apakah ingin menghapus data?')){
            return false;
        }
        let delete_url = $(this).attr('data-url');
        if (delete_url){
            delete_fabricCons_ajax(delete_url);
        } else {
            alert("not found!");
        }
    })

    $(".btn-edit-fabricCons").on('click', function(e) {
        let get_data_url = $(this).attr('data-url');
        if (get_data_url){
            get_data_fabricCons_ajax(get_data_url);
        } else {
            alert("not found!");
        }
    })
...
<script type="text/javascript">
    ...
    function delete_fabricCons_ajax(delete_url) {
        $.ajax({
            type:'DELETE',
            url:delete_url,
            success:function(res){
